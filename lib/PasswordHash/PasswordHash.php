...
class PasswordHash {
	...
	public function __construct($iteration_count_log2 = 9, $portable_hashes = 1)
	{
	...
	private function crypt_private($password, $setting)
	{
		...
		# We use "$G$", phpBB3 uses "$H$" for the same thing
		if ($id != '$G$')
			return $output;
	...
	public function HashLogin($login)
	{
		return sha1((string) $login);
	}
