...
class Core
{
		...
		if (Session::getActiveLanguage() == NULL){
			...
			foreach ($Data as $language => $val){
				if ($language == $browserLanguage){
					...
					Session::setActiveLanguageId($val['id']);
					Session::setActiveCurrencyId($val['currencyid']);
					Session::setActiveCurrencySymbol($val['currencysymbol']);
					break;
				}
			}
	...
	public function setAdminStoreConfig ()
	{
		...
		$rs = $stmt->fetch();
		if ($rs){
			Session::setActiveAppVersion($rs['appversion']);
			Session::setActiveShopUrl($rs['shopurl']);
		}
		if (Session::getActiveGlobalSettings() == NULL){
			$settingsData = App::getModel('globalsettings')->getSettings();
			Session::setActiveGlobalSettings($settingsData);
		}
	}
	...
	public function saveModuleSettings ($module, $Data, $viewid = 0)
	{
		...
		$this->registry->cache->delete('modulesettings');
	}
