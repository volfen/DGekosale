...
class GalleryModel extends FileUploader
{
	...
	public function getSettings ()
	{
		...
		try{
			...
			throw new CoreException(_('ERR_GALLERYSETTINGS_EDIT'), 18, $e->getMessage());
		}
	...
	public function process ($file, $proportion = 1)
	{
		...
		if ($Data['error'] == 0 && isset($Data['type'])){
			try{
				...
				throw new CoreException(_('ERR_SAVE_FILE'), 23, $e->getMessage());
			}
		}
		else{
			$error = _($this->__ERROR_MSG[$Data['error']]);
			switch ($Data['error']) {
				case 1:
					$error .= ' (' . _('MAX_UPLOAD_FILE_SIZE_IS') . ': ' . ini_get('upload_max_filesize') . ')';
					break;
			}
	...
	public function __call ($name, $values)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_FILE_NOT_EXIST'), 8, 'Gallery file doesn\'t exists on disk');
		}
