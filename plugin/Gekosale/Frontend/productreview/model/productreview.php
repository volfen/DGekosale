...
class ProductReviewModel extends Component\Model
{
	...
	{
		if ($count == 0){
			return sprintf(_('TXT_OPINIONS_QTY0'), $count);
		}
		if ($count == 1){
			return sprintf(_('TXT_OPINIONS_QTY1'), $count);
		}
		else{
			$sufix = $count % 100;
			if ($sufix > 11 && $sufix < 15){
				return sprintf(_('TXT_OPINIONS_QTY2'), $count);
			}
			else{
				$modulo = $sufix % 10;
				switch ($modulo) {
					...
					case 4:
					default:
						return sprintf(_('TXT_OPINIONS_QTY3'), $count);
				}
			}
		}
	}
