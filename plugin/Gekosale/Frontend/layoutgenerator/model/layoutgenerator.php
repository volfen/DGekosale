...
class LayoutGeneratorModel extends Component\Model
{
	...
	public function LoadLayout ($subpageName)
	{
		...
		$this->_columns = Array();
		if (($this->_columns = $this->registry->cache->load('columns' . $subpageName)) === FALSE){
			$query = '
						SELECT
							SLC.order AS `column`,
							SLC.width AS width,
			...
			$this->registry->cache->save('columns' . $subpageName, $this->_columns);
		}
	}
		...
		foreach ($this->_columns as $i => $column){
			...
			foreach ($column['boxes'] as $box){
				...
				if (isset($box['params']['js']['iEnableBox'])){
					...
					if ($box['params']['js']['iEnableBox'] == 1 && Session::getActiveClientid() > 0){
						$showBox = true;
					}
					
					if ($box['params']['js']['iEnableBox'] == 2 && Session::getActiveClientid() == NULL){
						$showBox = true;
					}
