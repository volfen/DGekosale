...
class NewsletterModel extends Component\Model
{
	...
	public function addAJAXClientAboutNewsletter ($email, $privacy)
	{
		...
		if (! preg_match('/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.(?:[A-Z]{2}|com|org|net|gov|mil|biz|info|mobi|name|aero|jobs|museum)$/i', $email)){
			return $objResponse->script('GError("' . _('TXT_WRONG_EMAIL') . '")');
		}
		
		if ($privacy == 0){
			return $objResponse->script('GError("' . _('ERR_NEWSLETTER_PRIVACY') . '")');
		}
		
		$checkEmailExists = $this->checkEmailIfExists($email);
		if ($checkEmailExists > 0){
			return $objResponse->assign("error", "innerHTML", _('ERR_EMAIL_NOT_EXISTS'));
		}
		else{
			...
			return $objResponse->assign("success", "innerHTML", _('TXT_RECEIVE_EMAIL_WITH_ACTIVE_LINK'));
		}
	}
	...
	public function updateNewsletterActiveLink ($idclientnewsletter, $email)
	{
		...
		try{
			...
			App::getModel('mailer')->sendEmail(Array(
				...
				'bcc' => false,
				'subject' => _('TXT_REGISTRATION_NEWSLETTER'),
	...
	public function deleteAJAXClientAboutNewsletter ($email)
	{
		...
		try{
			if ($email == NULL){
				$objResponse->assign("info", "innerHTML", '<strong><font color = "red">' . _('ERR_DELETE_CLIENT_FROM_NEWSLETTER') . '<br> ' . _('ERR_EMPTY_EMAIL_FORM_LOGIN') . '</font></strong>');
			}
			else{
				...
				if ($checkmail == true){
					...
					if ($checkEmailExists > 0){
						...
						$objResponse->assign("info", "innerHTML", '<strong><font color = "green">' . _('TXT_RECEIVE_EMAIL_WITH_DEACTIVE_LINK') . '</font></strong>');
						$objResponse->assign('newsletterformphrase', 'value', '');
					}
					else{
						$objResponse->assign("info", "innerHTML", '<strong><font color = "red">' . _('ERR_EMAIL_NO_EXISTS') . '</font></strong>');
						$objResponse->assign('newsletterformphrase', 'value', '');
					}
				}
				else{
					$objResponse->assign('newsletterformphrase', 'value', '');
					$objResponse->assign("info", "innerHTML", '<strong><font color = "red">' . _('ERR_WRONG_FORMAT') . '</font></strong>');
				}
			}
		}
		catch (Exception $fe){
			$objResponse->assign("info", "innerHTML", '<strong><font color = "red">' . _('ERR_DELETE_CLIENT_FROM_NEWSLETTER') . '</font></strong>');
		}
		return $objResponse;
	}
