...
class NewsletterBoxController extends Component\Controller\Box
{
	...
	public function index ()
	{
		...
		$form->AddChild(new SimpleForm\Elements\Checkbox(Array(
			'name' => 'confirmterms',
			'label' => _('TXT_NEWSLETTER_ACCEPT') . ' <a href="' . $url . '" target="_blank">' . _('TXT_NEWSLETTER_CONDITIONS') . '</a>',
			'rules' => Array(
				new SimpleForm\Rules\RequiredDependency(_('TXT_NEWSLETTER_ACCEPT_CONDITIONS'), $action, new SimpleForm\Conditions\Equals('1'))
			),
			'default' => 0
		)));
		...
		if ($form->Validate()){
			...
			if ($formData['action'] == 1){
				...
				if ($checkEmailExists > 0){
					$this->registry->template->assign('signup_error', _('ERR_EMAIL_NOT_EXISTS'));
				}
				else{
					...
					$this->registry->template->assign('signup_success', _('TXT_RECEIVE_EMAIL_WITH_ACTIVE_LINK'));
				}
			}
			...
			if ($formData['action'] == 2){
				...
				if ($checkEmailExists > 0){
					...
					$this->registry->template->assign('signup_success', _('TXT_RECEIVE_EMAIL_WITH_DEACTIVE_LINK'));
				}
				else{
					$this->registry->template->assign('signup_error', _('ERR_EMAIL_NO_EXIST'));
				}
			}
		}
