...
class RulesCartController extends Component\Controller\Admin
{
	...
	public function index ()
	{
		...
		$tree->AddChild(new FormEngine\Elements\SortableList(Array(
			...
			'add_item_prompt' => _('TXT_ENTER_NEW_CART_RULE_NAME'),
			'delete_item_prompt' => _('TXT_DELETE_CART_RULE'),
			'sortable' => false,
			'selectable' => false,
			'clickable' => true,
			'deletable' => true,
	...
	public function edit ()
	{
		...
		$languageData->AddChild(new FormEngine\Elements\TextField(Array(
			...
			'rules' => Array(
				...
				new FormEngine\Rules\LanguageUnique(_('ERR_NAME_ALREADY_EXISTS'), 'rulescarttranslation', 'name', null, Array(
					'column' => 'rulescartid',
		...
		$additionalData = $form->AddChild(new FormEngine\Elements\Fieldset(Array(
			...
			'label' => _('TXT_PROMOTIONRULE_DISCOUNT_DATA')
		)));
		...
		$additionalData->AddChild(new FormEngine\Elements\TextField(Array(
			...
			'label' => _('TXT_VALUE'),
			'rules' => Array(
				new FormEngine\Rules\Format(_('ERR_VALUE_INVALID'), '/^([0-9]*\.*([0-9]{1,2})|(\0)?)$/')
			),
			'dependencies' => Array(
				new FormEngine\Dependency(FormEngine\Dependency::HIDE, $discountForAll, new FormEngine\Conditions\Not(new FormEngine\Conditions\Equals('1')))
			)
		...
		foreach ($clientGroups as $clientGroup){
			...
			$additionalData->AddChild(new FormEngine\Elements\TextField(Array(
				...
				'rules' => Array(
					new FormEngine\Rules\Format(_('ERR_VALUE_INVALID'), '/^([0-9]*\.*([0-9]{1,2})|(\0)?)$/'),
					new FormEngine\Rules\Custom(_('ERR_VALUE_INVALID'), Array(
						$this,
						'checkDiscountValue'
					), Array(
						'suffixType' => $suffix
		...
		if (count($deliverers)){
			...
			$deliverersData->AddChild(new FormEngine\Elements\StaticText(Array(
				'text' => '<p><strong>' . _('TXT_EMPTY_DISPATCHMETHODS') . '</strong><br/>
		...
		if (count($payments)){
			...
			$paymentsData->AddChild(new FormEngine\Elements\StaticText(Array(
				'text' => '<p><strong>' . _('TXT_EMPTY_PAYMENTMETHODS') . '</strong><br/>
		...
		$pricePane = $form->AddChild(new FormEngine\Elements\Fieldset(Array(
			'name' => 'price_pane',
			'label' => _('TXT_SUM_PRICE')
		)));

		$pricePane->AddChild(new FormEngine\Elements\StaticText(Array(
			'text' => '<p><strong>' . _('TXT_FINAL_CART_PRICE') . '</strong></p>'
		)));

		$pricePane->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'cart_price_from',
			'label' => _('TXT_PRICE_FROM'),
			'suffix' => 'brutto',
			'rules' => Array(
				new FormEngine\Rules\Format(_('ERR_NUMERIC_INVALID'), '/^([0-9]*\.*([0-9]{1,2})|(\0)?)$/')
			)
		)));

		$pricePane->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'cart_price_to',
			'label' => _('TXT_PRICE_TO'),
			'suffix' => 'brutto',
			'rules' => Array(
				new FormEngine\Rules\Format(_('ERR_NUMERIC_INVALID'), '/^([0-9]*\.*([0-9]{1,2})|(\0)?)$/')
			)
		...
		$pricePane->AddChild(new FormEngine\Elements\StaticText(Array(
			'text' => '<p><strong>' . _('TXT_TOTAL_COST') . '</strong></p>'
		)));
		...
		$rulesCartRuleData['required_data'] = Array(
			...
			'discount' => round($rawRulesCartData['discount'], 0),
