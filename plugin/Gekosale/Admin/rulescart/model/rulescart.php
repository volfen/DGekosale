...
class RulesCartModel extends Component\Model
{
	...
	{
		$data = Array(
			'name' => (isset($request['name']) && strlen($request['name'])) ? $request['name'] : _('TXT_NEW_RULES_CART')
		);
		return Array(
	...
	public function changeRulesCartOrder ($request)
	{
		...
		return Array(
			'status' => _('TXT_RULE_CART_ORDER_SAVED')
		);
	}
	...
	public function editRulesCart ($submitedData, $idRulesCart)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_RULE_CART_EDIT'), 3002, $e->getMessage());
		}
	...
	public function rulesCartEdit ($submitedData, $idRulesCart)
	{
		...
		foreach ($submitedData['name'] as $key => $val){
			...
			try{
				...
				throw new CoreException(_('ERR_RANGETYPE_TRANSLATION_EDIT'), 4, $e->getMessage());
			}
		}
			...
			if (isset($submitedData['discountforall']) && $submitedData['discountforall'] == 1){
				foreach ($clientGroups as $clientGroup){
					...
					try{
						...
						throw new CoreException(_('ERR_UPDATE_CLIENTGROUP_RULE_CART'), 112, $e->getMessage());
					}
				}
			}
			else{
				foreach ($clientGroups as $clientGroup){
					if (isset($submitedData['groupid_' . $clientGroup['id']]) && $submitedData['groupid_' . $clientGroup['id']] > 0){
						...
						try{
							...
							throw new CoreException(_('ERR_UPDATE_CLIENTGROUP_RULE'), 112, $e->getMessage());
						}
					}
				}
			}
	...
	public function addRulesCart ($Data)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_CART_RULE_ADD'), 3003, $e->getMessage());
		}
