...
class MigrationModel extends Component\Model
{
	...
	public function doProcessQueque ($request)
	{
		...
		switch ($params['entity']) {
			case 1:
				$Data = $this->registry->cache->load('migration_products');
				$id = array_shift($Data);
				$response = $this->getProduct($id);
				if (isset($response['product'])){
					$this->addUpdateProduct($response['product']);
				}
				$this->registry->cache->save('migration_products', $Data);
				break;
			case 2:
				$Data = $this->registry->cache->load('migration_categories');
				$id = array_shift($Data);
				...
				$this->registry->cache->save('migration_categories', $Data);
				break;
			case 3:
				$Data = $this->registry->cache->load('migration_producers');
				$id = array_shift($Data);
				...
				$this->registry->cache->save('migration_producers', $Data);
				break;
			case 4:
				...
				break;
			case 5:
				$Data = $this->registry->cache->load('migration_attributes');
				$id = array_shift($Data);
				...
				$this->registry->cache->save('migration_attributes', $Data);
				break;
			case 6:
				$Data = $this->registry->cache->load('migration_orders');
				$id = array_shift($Data);
				$response = $this->getOrder($id);
				if (isset($response['order'])){
					$this->addOrder($response['order']);
				}
				$this->registry->cache->save('migration_orders', $Data);
				break;
			case 7:
				$Data = $this->registry->cache->load('migration_similarproducts');
				$id = array_shift($Data);
				...
				Db::getInstance()->commit();
				$this->registry->cache->save('migration_similarproducts', $Data);
				break;
		}
	...
	protected function addUpdateAttributes ($Data)
	{
		...
		$data = array(
			'attributegroupname' => Session::getActiveMigrationGroupName(),
			'category' => '',
			'attributes' => array(
				'editor' => array(
					0 => array(
		...
		$id = Session::getActiveMigrationGroupId();
	...
	protected function addUpdateProduct ($Data)
	{
		...
		if ($rs){
			...
			if (! empty($attributes)){
				...
				$attributes = array(
					'set' => Session::getActiveMigrationGroupId()
				) + $allAttributes;
	...
	protected function addUpdateSimilarProducts ($mainProduct, $products)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_SIMILARPRODUCT_ADD'), $e->getMessage());
		}
