...
class ProductModel extends Component\Model\Datagrid
{
	...
	public function deleteProduct ($id)
	{
		...
		if (count($recordsNotDeleted) > 0){
			return Array(
				'error' => _('ERR_BIND_PRODUCT_ORDER') . ': ' . implode(',', $recordsNotDeleted)
			);
		}
		else{
			return true;
	...
	public function productUpdateAll ($Data, $id)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_PRODUCT_EDIT'), 3002, $e->getMessage());
		}
	...
	protected function updateProductNew ($Data, $id)
	{
		...
		if ($Data['newactive'] == 1){
			...
			try{
				...
				throw new CoreException(_('ERR_PRODUCT_NEW_ADD'), 112, $e->getMessage());
			}
		}
	}
	...
	public function productUpdate ($Data, $id)
	{
		...
		if ($Data['delivererid'] > 0){
			...
			try{
				...
				throw new CoreException(_('ERR_DELIVERER_PRODUCT_ADD'), 11, $e->getMessage());
			}
		}
			...
			if (is_array($attributegroup)){
				if (substr($key, 0, 3) == 'new'){
					...
					try{
						...
						throw new CoreException(_('ERR_PRODUCT_ATTRIBUTES_ADD'), 112, $e->getMessage());
					}
	...
	protected function deleteAttributesProductSet ($productid, $variants)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_PRODUCT_ATTRIBUTE_DELETE'), 112, $e->getMessage());
		}
	}
	...
	public function addProductTranslation ($Data, $productid)
	{
		foreach ($Data['name'] as $key => $val){
			...
			try{
				...
				throw new CoreException(_('ERR_PRODUCT_TRANSLATION_ADD'), 112, $e->getMessage());
			}
		}
	}
			...
			foreach ($array as $key => $photo){
				if (! is_array($photo) && is_int($key)){
					...
					try{
						...
						throw new CoreException(_('ERR_PRODUCT_PHOTO_ADD'), 112, $e->getMessage());
					}
				}
			}
		}
		...
		foreach ($Data as $key => $file){
			if (is_int($key) && $file != 0){
				...
				try{
					...
					throw new CoreException(_('ERR_PRODUCT_FILE_ADD'), 112, $e->getMessage());
				}
			}
		}
	}
	...
	public function addProductToCategoryGroup ($array, $ProductId)
	{
		foreach ($array as $value){
			...
			try{
				...
				throw new CoreException(_('ERR_PRODUCT_CATEGORY_ADD'), 112, $e->getMessage());
			}
		}
	}
	...
	public function getProductVariant ($attributes, $contener)
	{
		foreach ($attributes as $key => $variant){
			...
			try{
				...
				throw new CoreException(_('ERR_PRODUCT_VARIANT_ADD'), 112, $e->getMessage());
			}
		}
	}
		...
		foreach ($clientGroups as $group){
			...
			if ((isset($Data['groupid_' . $clientgroupid]) && $Data['groupid_' . $clientgroupid] == 1) || (isset($Data['promotion_' . $clientgroupid]) && $Data['promotion_' . $clientgroupid] == 1)){
				...
				try{
					...
					throw new CoreException(_('ERR_PRODUCT_GROUP_PRICE_ADD'), 112, $e->getMessage());
				}
			}
		}
	}
