...
class LanguageForm extends Component\Form
{
	...
	public function initForm ()
	{
		...
		$requiredData->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'name',
			'label' => _('TXT_NAME'),
			'comment' => _('TXT_EXAMPLE') . ': en_EN',
			'rules' => Array(
				new FormEngine\Rules\Required(_('ERR_EMPTY_NAME')),
				new FormEngine\Rules\Unique(_('ERR_NAME_ALREADY_EXISTS'), 'language', 'name', null, Array(
					'column' => 'idlanguage',
		...
		$requiredData->AddChild(new FormEngine\Elements\TextField(Array(
			...
			'comment' => _('TXT_EXAMPLE') . ': TXT_ENGLISH',
			'rules' => Array(
		...
		if ($this->registry->core->getParam() == ''){
			$requiredData->AddChild(new FormEngine\Elements\Select(Array(
				'name' => 'copylanguage',
				'label' => _('TXT_COPY_FROM_LANGUAGE'),
