...
class ProductNewsModel extends Component\Model\Datagrid
{
	...
	public function doAJAXEnableProductNews ($datagridId, $id)
	{
		try{
			...
			$objResponse->script("GF_Alert('{_('ERR_UNABLE_TO_ENABLE_PRODUCT_NEWS')}', '{$e->getMessage()}');");
			return $objResponse;
		}
	}
	...
	public function doAJAXDisableProductNews ($datagridId, $id)
	{
		try{
			...
			$objResponse = new xajaxResponse();
			$objResponse->script("GF_Alert('{_('ERR_UNABLE_TO_DISABLE_PRODUCT_NEWS')}', '{$e->getMessage()}');");
			return $objResponse;
		}
	}
