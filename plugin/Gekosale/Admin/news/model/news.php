...
class NewsModel extends Component\Model\Datagrid
{
	...
	public function doAJAXEnableNews ($datagridId, $id)
	{
		try{
			...
			$objResponse->script("GF_Alert('{_('ERR_UNABLE_TO_ENABLE_STATICBLOCKS')}', '{$e->getMessage()}');");
			return $objResponse;
		}
	}
			...
			foreach ($Data['photo'] as $key => $photo){
				if (! is_array($photo) && is_int($key) && ($photo > 0)){
					...
					try{
						...
						throw new CoreException(_('ERR_NEWS_PHOTO_UPDATE'), 112, $e->getMessage());
					}
				}
			}
		}
	...
	public function addNewNews ($Data)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_CONTACT_ADD'), 11, $e->getMessage());
		}
			...
			foreach ($array as $key => $photo){
				if (! is_array($photo) && is_int($key)){
					...
					try{
						...
						throw new CoreException(_('ERR_news_PHOTO_ADD'), 112, $e->getMessage());
					}
				}
			}
		}
	...
	public function addNewsTranslation ($Data, $id)
	{
		foreach ($Data['topic'] as $key => $val){
			...
			try{
				...
				throw new CoreException(_('ERR_NEWS_TRANSLATION_ADD'), 4, $e->getMessage());
			}
		}
	}
	...
	public function addNewsView ($Data, $id)
	{
		foreach ($Data as $value){
			...
			try{
				...
				throw new CoreException(_('ERR_NEWS_VIEW_ADD'), 4, $e->getMessage());
			}
		}
	}
	...
	public function editNews ($Data, $id)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_CONTACT_EDIT'), 125, $e->getMessage());
		}
	...
	public function updateNews ($publish, $featured, $id, $startdate, $enddate)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_NEWS_EDIT'), 13, $e->getMessage());
		}
	}
