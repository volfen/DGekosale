...
class AttributeGroupModel extends Component\Model
{
	...
	public function addEmptyGroup ($request)
	{
		if (! isset($request['name']) || ! strlen($request['name'])){
			$autoNameBase = _('TXT_NEW_ATTRIBUTE_GROUP');
		...
		if ($rs['total'] == 0){
			...
			return Array(
				'error' => _('ERR_ATTRIBUTE_PRODUCT_GROUP_ALREADY_EXISTS')
			);
		}
	...
	public function updateAttributeGroupName ($Data, $id)
	{
		...
		try{
			...
			throw new Exception(_('ERR_ATTRIBUTE_GROUP_UPDATE'), 1, $e->getMessage());
			return false;
		}
		return true;
	}
	...
	public function DeleteAttribute ($id)
	{
		...
		if ($rs) {
			throw new Exception(_('ERR_BIND_ATTRIBUTE_VALUE'));
		}
