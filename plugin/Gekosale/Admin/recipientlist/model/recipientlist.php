...
class RecipientListModel extends Component\Model\Datagrid
{
	...
	public function addNewRecipient ($Data)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_NEWSLETTER_ADD'), 112, $e->getMessage());
		}
	...
	public function addNewClientgroupList ($Data, $id)
	{
		foreach ($Data['clientgroup'] as $value){
			...
			try{
				...
				throw new CoreException(_('ERR_CLIENT_GROUP_ADD'), 112, $e->getMessage());
			}
		}
	}
	...
	public function addNewRecipientName ($Data)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_RECIPIENT_LIST_ADD'), 11, $e->getMessage());
		}
	...
	public function getRecipientListView ($idrecipientlist)
	{
		...
		if ($rs){
			...
			throw new CoreException(_('ERR_RECIPIENT_LIST_NO_EXIST'));
		}
		return $Data;
	}
	...
	public function updateRecipientList ($Data, $recipientlistid)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_RECEPIENTLIST_EDIT'), 13, $e->getMessage());
			return false;
		}
	}
		...
		if ( !empty($Data['clientgroup']) && is_array($Data['clientgroup'])) {
			foreach ($Data['clientgroup'] as $key => $val){
				...
				try{
					...
					throw new CoreException(_('ERR_RECIPIENT_CLIENT_GROUP_LIST_EDIT'), 13, $e->getMessage());
				}
			}
		}
		...
		if ( !empty($Data['clients']) && is_array($Data['clients'])) {
			foreach ($Data['clients'] as $key => $val){
				...
				try{
					...
					throw new CoreException(_('ERR_RECIPIENT_CLIENT_LIST_EDIT'), 13, $e->getMessage());
				}
			}
		}
