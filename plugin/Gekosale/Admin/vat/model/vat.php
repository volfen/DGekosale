...
class VatModel extends Component\Model\Datagrid
{
	...
	public function deleteVAT ($id)
	{
		...
		if ($rs['total'] > 0){
			...
			if ($productTotal == 0){
				...
				return Array(
					'error' => _('ERR_VAT_BIND_TO_PRODUCT')
				);
			}
		}
		else{
			return Array(
				'error' => _('ERR_DELETE_ONLY_VAT')
			);
		}
	}
	...
	public function editVAT ($Data, $id)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_VAT_EDIT'), 125, $e->getMessage());
		}
	...
	public function updateVatTranslation ($Data, $id)
	{
		...
		foreach ($Data as $key => $value){
			...
			try{
				...
				throw new CoreException(_('ERR_VAT_TRANSLATION_ADD'), 4, $e->getMessage());
			}
		}
	}
	...
	public function addNewVAT ($Data)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_VAT_ADD'), 11, $e->getMessage());
		}
