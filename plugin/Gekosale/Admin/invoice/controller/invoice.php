...
class invoiceController extends Component\Controller\Admin
{
	...
	public function add ()
	{
		...
		$invoiceData->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'salesperson',
			'label' => _('TXT_SALES_PERSON'),
			'rules' => Array(
				new FormEngine\Rules\Required(_('ERR_EMPTY_SALES_PERSON'))
			),
		...
		$invoiceDate = $invoiceData->AddChild(new FormEngine\Elements\Date(Array(
			'name' => 'invoicedate',
			'label' => _('TXT_INVOICE_DATE'),
			'rules' => Array(
				new FormEngine\Rules\Required(_('ERR_EMPTY_INVOICE_DATE'))
			),
			'default' => date('Y-m-d')
		)));
		...
		elseif (! empty($inFaktSettings) && $inFaktSettings['infaktlogin'] != '' && $inFaktSettings['infaktpassword'] != ''){
			...
			$invoiceData->AddChild(new FormEngine\Elements\TextField(Array(
				'name' => 'invoicenumber',
				'label' => _('TXT_INVOICE_NUMBER'),
				'rules' => Array(
					new FormEngine\Rules\Required(_('ERR_EMPTY_INVOICE_NUMBER'))
				),
		...
		$invoiceData->AddChild(new FormEngine\Elements\Date(Array(
			'name' => 'duedate',
			'label' => _('TXT_MATURITY'),
			'rules' => Array(
				new FormEngine\Rules\Required(_('ERR_EMPTY_MATURITY'))
			),
		...
		$invoiceData->AddChild(new FormEngine\Elements\Textarea(Array(
			'name' => 'comment',
			'label' => _('TXT_COMMENT'),
			'default' => _('TXT_ORDER') . ': ' . $orderData['order_id']
		)));
		
		$invoiceData->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'totalpayed',
			'label' => _('TXT_TOTAL_PAYED'),
			'rules' => Array(
				new FormEngine\Rules\Required(_('ERR_EMPTY_TOTAL_PAYED'))
			),
			'default' => '0.00',
