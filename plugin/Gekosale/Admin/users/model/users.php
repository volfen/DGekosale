...
class usersModel extends Component\Model\Datagrid
{
	...
	public function doAJAXEnableUser ($datagridId, $id)
	{
		try{
			...
			$objResponse->script("GF_Alert('{_('ERR_UNABLE_TO_ENABLE_USER')}', '{$e->getMessage()}');");
			return $objResponse;
		}
	}
	...
	{
		if ($id == Session::getActiveUserid()){
			throw new Exception(_('ERR_CAN_NOT_DISABLE_YOURSELF'));
		}
		$sql = 'UPDATE user SET
					active = 0
					WHERE iduser = :id';
	...
	{
		if ($id == Session::getActiveUserid()){
			throw new Exception(_('ERR_CAN_NOT_ENABLE_YOURSELF'));
		}
		$sql = 'UPDATE user SET
					active = 1
					WHERE iduser = :id';
	...
	protected function updateUserLogin ()
	{
		try{		
			...
			throw new CoreException(_('ERR_LOGINUSER_UPDATE'), 17, $e->getMessage());
		}
	}
	...
	public function updateUserPassword ($password)
	{
		if (isset($password) && ! empty($password)){
			...
			try{
				...
				throw new CoreException(_('ERR_PASSWORDUSER_UPDATE'), 18, $e->getMessage());
			}
		}
	}
	...
	public function updateUser ($Data, $userId)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_USER_UPDATE'), 118, $e->getMessage());
		}
		
		Db::getInstance()->commit();
		if ($userId == Session::getActiveUserid()){
			// Session::flush();
			// App::redirect('login');
		}
	}
	...
	protected function updateUserActive ($active)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_USER_ACTIVE_UPDATE'), 19, $e->getMessage());
		}
	}
	...
	protected function updateUserData ($Data, $userid)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_USERDATA_UPDATE'), 19, $e->getMessage());
		}
	}
	...
	public function addNewUser ($Data, $password)
	{
		...
		try{
			...
			throw new CoreException(_('ERR_USER_ADD'), 21, $e->getMessage());
		}
	...
	public function getUserFullName ()
	{
		if (Session::getActiveUserid() !== NULL){
			$fullName = Session::getActiveUserFirstname() . ' ' . Session::getActiveUserSurname();
			return $fullName;
		}
	}
	...
	public function setLoginTime ()
	{
		...
		try{
			...
			throw new CoreException(_('ERR_LOGINTIME'), 22, $e->getMessage());
		}
	}
