...
<article class="article">
	...
	<table class="table table-striped order">
		...
		<tbody>
			...
			<tr>
				<td><strong>{{ orderproduct.productname|raw }}</strong>
				{% for attribute in orderproduct.attributes %}						
					<br />{{ attribute.attributegroup }}: {{ attribute.attributename }}
				{% endfor %}
				</td>
				<td>{{ orderproduct.price|priceFormat({'symbol': order.currencysymbol}) }}</td>
                <td><strong>{{ orderproduct.qty|number_format }}</strong></td>
				<td><strong>{{ orderproduct.qtyprice|priceFormat({'symbol': order.currencysymbol}) }}</strong></td>
			</tr>
			{% endfor %}
		</tbody>
		<tfoot>
			<tr>
				...
				<td><strong>{{ order.dispatchmethodprice|priceFormat({'symbol': order.currencysymbol}) }}</strong></td>
			</tr>
		</tfoot>
	</table>
	<div class="order-total">
    	<span class="total">{% trans %}TXT_ALL_ORDERS_PRICE{% endtrans %}</span>
        <span class="price">{{ order.globalprice|priceFormat({'symbol': order.currencysymbol}) }}</span>
	</div>
