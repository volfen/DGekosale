...
<div class="control-group {% if field.errors|length %}error{% endif %}">
    <div class="controls">
	<input type="hidden" name="{{ field.attributes.name }}" class="{{ class }}" id="{{ field.attributes.id }}" value="{{ field.value }}" />
		{% if field.errors|length %}<span class="help-block pull-left">{{ field.errors|join }}</span><br />{% endif %}
	</div>
</div>
{% endautoescape %}
{% endmacro %}
...
<div class="control-group {% if field.errors|length %}error{% endif %}">
	<label class="control-label {% if field.attributes.required == 1 %}required{% endif %}" for="{{ field.attributes.id }}">{{ field.attributes.label }}</label>
	<div class="controls">
		<select name="{{ field.attributes.name }}" id="{{ field.attributes.id }}">
